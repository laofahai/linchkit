...
if (typeof globalThis.localStorage === "undefined") {
  Object.defineProperty(globalThis, "localStorage", {
    value: { getItem: (k: string) => _store.get(k) ?? null, setItem: (k: string, v: string) => _store.set(k, v), removeItem: (k: string) => _store.delete(k), clear: () => _store.clear(), get length() { return _store.size; }, key: (i: number) => [..._store.keys()][i] ?? null },
    configurable: true,
  });
}
...
function makeIntent(overrides: Partial<IntentResolution> = {}): IntentResolution {
  return { action: "primary_action", schema: "primary_entity", input: { x: 1 }, missingFields: [], confidence: 0.6, explanation: "Primary explanation", actionLabel: "Primary", actionDescription: "Primary description", inputSchema: { x: { type: "number", required: false } }, ...overrides };
}
function makeAlt(overrides: Partial<IntentAlternative> = {}): IntentAlternative {
  return { action: "alt_action", input: { y: 2 }, confidence: 0.55, missingFields: [], explanation: "Alt explanation", ...overrides };
}

describe("swapAlternative", () => {
  test("returns null when alternatives is undefined", () => { expect(swapAlternative(makeIntent({ alternatives: undefined }), 0)).toBeNull(); });
  test("returns null when alternatives is empty", () => { expect(swapAlternative(makeIntent({ alternatives: [] }), 0)).toBeNull(); });
  ...
  test("promotes the chosen alternative into the primary slot", () => {
    const alt = makeAlt({ action: "approve_order", input: { id: "o1" }, confidence: 0.5, missingFields: ["reason"], explanation: "Approve order o1" });
  ...
  test("demotes the previous primary into alternatives, preserving display metadata", () => {
    const intent = makeIntent({ action: "primary_action", schema: "primary_entity", actionLabel: "Primary", actionDescription: "Primary description", inputSchema: { x: { type: "number", required: false } }, input: { x: 1 }, confidence: 0.6, explanation: "Primary explanation", missingFields: ["m"], alternatives: [makeAlt({ action: "alt_a", confidence: 0.55 })] });
  ...
  test("swap is reversible", () => {
    const intent = makeIntent({ action: "primary_action", schema: "primary_entity", actionLabel: "Primary", actionDescription: "Primary description", inputSchema: { x: { type: "number", required: false } }, input: { x: 1 }, confidence: 0.6, missingFields: [], explanation: "Primary explanation", alternatives: [makeAlt({ action: "alt_a", confidence: 0.55, input: { y: 2 } })] });
  ...
  test("swap-IN uses backend-enriched alternative metadata when present", () => {
    const next = swapAlternative(makeIntent({ alternatives: [makeAlt({ action: "alt_a", confidence: 0.55, input: { id: "x" }, schema: "alt_entity", actionLabel: "Alternative A", actionDescription: "Does the alternative thing", inputSchema: { id: { type: "string", required: true } } })] }), 0);
  ...
  test("preserves remaining alternatives sorted by confidence DESC", () => {
    const intent = makeIntent({ action: "primary_action", confidence: 0.65, alternatives: [makeAlt({ action: "alt_high", confidence: 0.6 }), makeAlt({ action: "alt_mid", confidence: 0.5 }), makeAlt({ action: "alt_low", confidence: 0.45 })] });
    const next = swapAlternative(intent, 2);
    expect(next?.action).toBe("alt_low");
    expect(next?.alternatives?.map((a) => a.action)).toEqual(["primary_action", "alt_high", "alt_mid"]);
  });
});

interface CapturedRequest { url: string; method?: string; body: unknown; headers: Record<string, string>; }
let captured: CapturedRequest | null;
let originalFetch: typeof fetch;
function installFetch(response: { status: number; body: unknown }) {
  originalFetch = globalThis.fetch;
  globalThis.fetch = (async (input: RequestInfo | URL, init?: RequestInit) => {
    captured = { url: typeof input === "string" ? input : (input as URL).toString(), method: init?.method, body: init?.body ? JSON.parse(init.body as string) : undefined, headers: (init?.headers as Record<string, string>) ?? {} };
...
describe("resolveIntent", () => {
  test("returns { kind: 'proposal' } when the server returns a proposal", async () => {
    const proposalView = makeIntent({ action: "submit_request", alternatives: [makeAlt({ action: "draft_request", confidence: 0.5 })] });
  ...
  test("returns { kind: 'unavailable' } on 503", async () => {
    installFetch({ status: 503, body: { success: false, error: { code: "AI.UNAVAILABLE", message: "AI service is not configured." } } });
  ...
  test("throws on non-2xx, non-503 responses", async () => {
    installFetch({ status: 500, body: { success: false, error: { code: "AI.INTERNAL", message: "boom" } } });
  ...
  test("forwards optional scope in the request body", async () => {
    installFetch({ status: 200, body: { proposal: null } });
    await resolveIntent("create a request", { entityFilter: ["request"], actionFilter: ["submit_request"] });
    expect(captured?.body).toEqual({ prompt: "create a request", scope: { entityFilter: ["request"], actionFilter: ["submit_request"] } });
  });
});
