...
export function ActiveFilter<TData, TType extends ColumnDataType>({
  ...
  return (
    <div className="relative flex h-7 items-center rounded-2xl border border-border bg-background shadow-xs text-xs">
...
export function ActiveFiltersMobileContainer({ children }: { children: React.ReactNode }) {
  ...
  // Set up ResizeObserver + MutationObserver to monitor container changes
  useEffect(() => {
    ...
    // Use MutationObserver to detect children changes
