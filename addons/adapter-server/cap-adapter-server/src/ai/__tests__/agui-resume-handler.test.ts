/**
 * AG-UI HITL resume handler unit tests (Spec 71 P2b) — core execution path.
 *
 ...
 * to stay under the 500-line guideline (#607); further split from the
 * authz/replay tests (agui-resume-handler-authz.test.ts).
 *
 * Covers: happy path, cancelled, §6.2 p2 swapped action, §6.2 p3 digest
 * mismatch, §5 expiry, and unsupported-status rejection.
 *
 * See agui-resume-handler-authz.test.ts for cross-user / replay / permission tests.
