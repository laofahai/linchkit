 ...
 *
 * Drives `runAgUiResume` directly against a REAL CommandLayer so the §6
 * anti-TOCTOU defenses are exercised end to end. Endpoint wiring lives in
 * agui-resume-endpoint.test.ts. Shared fixtures are in helpers/agui-resume-helpers.ts.
 *
 ...
 *  - CommandLayer permission DENY → normal finish (error surfaced), no record
 */

import { describe, expect, test } from "bun:test";
import { type AGUIEvent, EventType, type ResumeEntry } from "@linchkit/cap-adapter-ag-ui";
...
import {
  ...
  resolvedResume,
  VIEWER,
} from "./helpers/agui-resume-helpers";
