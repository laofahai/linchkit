...
Only include fields where you have genuine confidence. Omit fields where you would be guessing.`;
    ...
    //
    // Note: `/api/ai/resolve-intent` lives in `./ai-resolve-intent.ts` —
    // it's the canonical Spec 52 §2.6 endpoint that wires the resolver
    // from `@linchkit/cap-ai-provider`. Keep these two endpoints separate:
    // resolve-intent is read-only (proposes), execute-intent writes.
