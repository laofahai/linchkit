...
const prSchema = defineSchema({
  ...
  fields: {
    ...
    // In production this is generated by generateLinkColumns(); here we declare it
...
describe.skipIf(!dbAvailable)("GraphQL Link resolvers (integration)", () => {
  ...
  beforeAll(async () => {
    ...
    await db?.execute(
      ...
      CREATE TABLE IF NOT EXISTS "link_pr" (
        ...
        "title" text NOT NULL,
        "amount" numeric,
        "link_dept_id" text REFERENCES "link_dept"("id")
      )
    `),
    );
  ...
  test("create department and purchase request linked via FK", async () => {
    ...
    // The FK column is `link_dept_id` (generated by link convention: {to}_id)
  ...
  test("forward navigation — purchase request resolves linked department", async () => {
    ...
    // Query purchase request with forward link field (link_dept)
    ...
    const resolvedDept = prRecord.link_dept as Record<string, unknown>;
  ...
  test("reverse navigation — department resolves linked purchase requests", async () => {
    ...
    await linkProvider.create("link_pr", {
      ...
      amount: 2000,
      link_dept_id: otherDeptId,
    });

    // Query department with reverse link field (link_prs — plural of from schema name)
    const result = await linkGql(`
      query {
        linkDept(id: "${deptId}") {
          id
    ...
    const purchaseRequests = deptRecord.link_prs as Array<Record<string, unknown>>;
  ...
  test("forward navigation — returns null when FK is not set", async () => {
    ...
    expect(prRecord.link_dept).toBeNull();
  });
