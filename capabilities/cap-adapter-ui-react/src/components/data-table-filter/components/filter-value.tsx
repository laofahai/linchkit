...
export const FilterValue = memo(FilterValueInner) as typeof FilterValueInner;

function FilterValueInner<TData, TType extends ColumnDataType>({
  filter,
  column,
  actions,
  strategy,
...
export function FilterValueTextDisplay<TData>({
  filter,
}: Omit<FilterValueDisplayProps<TData, "text">, "locale">) {
  if (!filter) return null;
...
export const FilterValueController = memo(
  FilterValueInnerController,
) as typeof FilterValueInnerController;

function FilterValueInnerController<TData, TType extends ColumnDataType>({
  filter,
  column,
  actions,
  strategy,
...
export function FilterValueNumberController<TData>({
  ...
  const setFilterOperatorWrapped = useCallback(
    (...args: unknown[]) => {
      actions.setFilterOperator(args[0] as string, args[1] as FilterModel<"number">["operator"]);
    },
    [actions],
  );
  const setFilterValueWrapped = useCallback(
    (...args: unknown[]) => {
      actions.setFilterValue(args[0] as Column<TData, "number">, args[1] as number[]);
    },
    [actions],
  );
  const setFilterOperatorDebounced = useDebounceCallback(setFilterOperatorWrapped, 500);
  const setFilterValueDebounced = useDebounceCallback(setFilterValueWrapped, 500);

  const changeNumber = (value: number[]) => {
    setValues(value);
    setFilterValueDebounced(column as Column<TData, "number">, value);
  };

  const changeMinNumber = (value: number) => {
    const newValues = createNumberRange([value, values[1] ?? 0]);
    setValues(newValues);
    setFilterValueDebounced(column as Column<TData, "number">, newValues);
  };
  ...
  const changeType = useCallback(
    (type: "single" | "range") => {
      ...
      if (type === "single")
        newValues = [values[0] ?? 0]; // Keep the first value for single mode
      ...
      const newOperator = type === "single" ? "eq" : "between";
