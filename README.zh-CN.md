...
> **AI 原生业务系统的「治理与演化」底座。** 系统的行为被绑定到你声明的意图——实体、动作、规则、制度。任何改动，无论出自人还是 AI，都先经「提议 → 审批」、并在执行时被强制；没有未经审阅的东西能进生产。更长远的一步：让系统从它被实际使用的方式里提议自己的改进——但永远先经人裁决，才毕业成被强制执行的规则。
...
更深的判断：模型只会越来越擅长*生成*系统。LinchKit 是让它们生成的东西、以及人改动的东西，**待在真正被强制执行（而不只是写在文档里）的规则之内**的那一层；并让系统**安全地演化**：一项被提议的改动（一条新规则、一条收紧的制度）——无论是人写的，还是系统从观察到的信号里浮现的——都先经人审阅裁决，才毕业成被强制执行的代码。AI 永不直接修改生产。（让这种一致性变得*可证明*、让经验→制度的成熟更自动，是这个底座所朝向的方向——见[愿景 spec](./docs/specs/00a_product_vision.md)。）
