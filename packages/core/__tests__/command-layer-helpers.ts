...
import {
	createCommandLayer,
	type CommandLayer,
} from "../src/engine/command-layer";
import { createActionExecutor, type ActionExecutor } from "../src/engine/action-engine";
...
export function createTestDataProvider() {
	...
	return {
		...
		async create(schema: string, input: Record<string, unknown>) {
			...
			const record = { id, ...input, _version: 1, tenant_id: null, created_at: new Date().toISOString(), updated_at: new Date().toISOString() };
