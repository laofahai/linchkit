	...
	describe("Slot ordering", () => {
		test("middlewares in the same slot execute by order (smaller first)", async () => {
			...
			layer.use({
				...
				slot: "pre",
				order: 300,
				handler: async (_ctx, next) => { executionOrder.push("pre_c"); await next(); },
			});
			layer.use({
				name: "pre_a",
				slot: "pre",
				order: 100,
				handler: async (_ctx, next) => { executionOrder.push("pre_a"); await next(); },
			});
			layer.use({
				name: "pre_b",
				slot: "pre",
				order: 200,
				handler: async (_ctx, next) => { executionOrder.push("pre_b"); await next(); },
			});
		...
		test("slots execute in correct pipeline order", async () => {
			...
			layer.use({
				name: "pre_mw",
				slot: "pre",
				handler: async (_ctx, next) => { executionOrder.push("pre"); await next(); },
			});
			layer.use({
				name: "auth_mw",
				slot: "auth",
			...
			layer.use({
				name: "perm_mw",
				slot: "permission",
				handler: async (_ctx, next) => { executionOrder.push("permission"); await next(); },
			});
			layer.use({
				name: "tenant_mw",
				slot: "tenant",
			...
			layer.use({
				name: "pre_action_mw",
				slot: "pre-action",
				handler: async (_ctx, next) => { executionOrder.push("pre-action"); await next(); },
			});
			layer.use({
				name: "post_action_mw",
				slot: "post-action",
				handler: async (_ctx, next) => { executionOrder.push("post-action"); await next(); },
			});
	...
	describe("Short-circuit on error", () => {
		...
		test("error in pre slot prevents subsequent slots", async () => {
			...
			layer.use({
				name: "auth",
				slot: "auth",
				handler: async (_ctx, next) => { authRan = true; await next(); },
			});
	...
	describe("Post-action slot", () => {
		test("post-action middleware receives execution result", async () => {
			...
			layer.use({
				name: "result_logger",
				slot: "post-action",
				handler: async (ctx, next) => { capturedResult = ctx.result; await next(); },
			});
		...
		test("post-action error does not affect result", async () => {
			...
			layer.use({
				name: "failing_post",
				slot: "post-action",
				handler: async () => { throw new Error("Post-action logging failed"); },
			});
	...
	describe("Tenant slot", () => {
		test("tenant middleware sets tenantId on context", async () => {
			...
			layer.use({
				...
				slot: "tenant",
				handler: async (ctx, next) => { ctx.tenantId = "tenant_abc"; await next(); },
			});
			layer.use({
				name: "capture",
				slot: "pre-action",
				handler: async (ctx, next) => { capturedTenantId = ctx.tenantId; await next(); },
			});
	...
	describe("Meta and headers", () => {
		test("headers are available in context", async () => {
			...
			layer.use({
				name: "header_check",
				slot: "pre",
				handler: async (ctx, next) => { capturedHeaders = ctx.headers; await next(); },
			});
		...
		test("meta allows middleware communication", async () => {
			...
			layer.use({
				name: "writer",
				slot: "pre",
				handler: async (ctx, next) => { ctx.meta.requestStart = Date.now(); await next(); },
			});
			layer.use({
				name: "reader",
				slot: "pre-action",
				handler: async (ctx, next) => { readTimestamp = ctx.meta.requestStart; await next(); },
			});
	...
	describe("Channel defaults", () => {
		test("defaults to 'internal' channel when not specified", async () => {
			...
			layer.use({
				name: "channel_check",
				slot: "pre",
				handler: async (ctx, next) => { capturedChannel = ctx.channel; await next(); },
			});
