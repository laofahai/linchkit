...
import { generateGraphQLObjectType, generateGraphQLInputType } from "../src/graphql";
...
describe("generateGraphQLObjectType", () => {
	...
	test("includes system fields with correct types", () => {
		...
		expect((fields.created_at.type as GraphQLNonNull<typeof GraphQLString>).ofType).toBe(GraphQLString);
	...
	test("maps field types correctly", () => {
		...
		expect((fields.priority.type as GraphQLNonNull<typeof GraphQLString>).ofType).toBe(GraphQLString);
...
describe("all field types", () => {
	...
	test("all storable field types produce GraphQL fields", () => {
		const expectedFields = [
			"f_string", "f_text", "f_number", "f_boolean",
			"f_date", "f_datetime", "f_enum", "f_json", "f_ref", "f_state",
		];
