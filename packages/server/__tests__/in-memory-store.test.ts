...
describe("InMemoryStore", () => {
	...
	test("get throws for non-existent record", async () => {
		await expect(store.get("task", "nope")).rejects.toThrow(
			"Record not found",
		);
	});
	...
	test("update throws for non-existent record", async () => {
		await expect(
			store.update("task", "nope", { title: "X" }),
	...
	test("delete removes a record", async () => {
		...
		await expect(store.get("task", "t1")).rejects.toThrow(
			"Record not found",
		);
	});

	test("delete throws for non-existent record", async () => {
		await expect(store.delete("task", "nope")).rejects.toThrow(
			"Record not found",
		);
	});
