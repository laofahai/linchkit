...
describe("E2E: Purchase management flow", () => {
	...
	test("3. Query via GraphQL — record exists", async () => {
		...
		const data = (result.data as Record<string, unknown>).purchaseRequest as Record<string, unknown>;
	...
	test("6. GraphQL list with pagination", async () => {
		...
		const list = (result.data as Record<string, unknown>).purchaseRequestList as Record<string, unknown>;
	...
	test("7. GraphQL typed mutation — submit via GraphQL", async () => {
		...
		const data = (result.data as Record<string, unknown>).submitPurchaseRequest as Record<string, unknown>;
		expect(data.status).toBe("pending");
	});
});
