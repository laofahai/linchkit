...
/**
 * Run the review gate script after a successful merge.
 * The script must output JSON matching ReviewGateResult: { verdict, reviews }.
 * Returns null if the gate passes (verdict === "PASS") or the script is unavailable.
 * Returns ReviewGateResult on FAIL so caller can surface issues.
 */
async function runReviewGate(
  repoRoot: string,
  scriptPath: string,
): Promise<ReviewGateResult | null> {
  const fullScript = scriptPath.startsWith("/") ? scriptPath : `${repoRoot}/${scriptPath}`;
  const file = Bun.file(fullScript);
  if (!(await file.exists())) {
    return null;
  }

  const proc = Bun.spawn([fullScript, "--json"], {
    cwd: repoRoot,
    stdout: "pipe",
    stderr: "pipe",
  });
  ...
  try {
    result = JSON.parse(stdout.trim()) as ReviewGateResult;
  } catch {
    // Script output is not valid JSON — treat non-zero exit as FAIL
    if (exitCode !== 0) {
      return { verdict: "FAIL", reviews: [] };
    }
    return null;
  }

  return result.verdict === "FAIL" ? result : null;
}

/**
 * Collect all ReviewIssue entries from a ReviewGateResult's reviews array.
 */
function collectReviewIssues(result: ReviewGateResult): ReviewIssue[] {
  return result.reviews.flatMap((r) => r.issues ?? []);
}
    ...
    async resolve(
      ...
      try {
        ...
        if (cleanResult.success) {
          if (options.config?.merge?.reviewGateEnabled) {
            const gateResult = await runReviewGate(repoRoot, options.config.merge.reviewGateScript);
            if (gateResult) {
              return {
                entry: { ...entry, status: "failed", resolvedTier: "review-fail" },
                success: false,
                tier: "review-fail",
                conflictFiles: [],
                errorMessage: "Review gate failed",
                warnings,
                reviewIssues: collectReviewIssues(gateResult),
              };
            }
          }
          ...
          if (autoResult.success) {
            if (options.config?.merge?.reviewGateEnabled) {
              const gateResult = await runReviewGate(
                repoRoot,
                options.config.merge.reviewGateScript,
              );
              if (gateResult) {
                return {
                  entry: { ...entry, status: "failed", resolvedTier: "review-fail" },
