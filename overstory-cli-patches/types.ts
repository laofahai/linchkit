...
export interface OverstoryConfig {
	...
	merge: {
		...
		/** Run the review gate script after each successful merge. Default: false. */
		reviewGateEnabled: boolean;
		/** Path to the review gate script (relative to project root). Default: .overstory/scripts/review-gate.sh. */
		reviewGateScript: string;
	};
...
export type ResolutionTier = "clean-merge" | "auto-resolve" | "ai-resolve" | "reimagine" | "review-fail";

/** A single issue reported by the review gate script. */
...
/** Structured result from the review gate script. */
...
export interface MergeResult {
	...
	warnings: string[];
	/** Issues reported by the review gate when tier is 'review-fail'. */
