...
High-traffic environments need sampling to control volume:
...
```
┌──────────┐    ┌──────────┐    ┌─────────────┐    ┌──────────────┐
│  Server   │───▶│  stdout  │───▶│  Collector  │───▶│  Storage     │
│  (Bun)    │    │  (JSON)  │    │  (Vector/   │    │  (Loki/ES/   │
│           │    │          │    │   Fluentd)  │    │   Postgres)  │
└──────────┘    └──────────┘    └─────────────┘    └──────────────┘

┌──────────┐    ┌──────────┐    ┌─────────────┐
│  Browser  │───▶│  POST    │───▶│  /api/logs  │──▶ Same pipeline
│  (React)  │    │  batch   │    │  endpoint   │
└──────────┘    └──────────┘    └─────────────┘
```

**Server logs:** Written to stdout as JSON lines. External collector (Vector, Fluentd, etc.) handles shipping. LinchKit does NOT embed a log shipper — that's infrastructure.
...
## 4. Frontend Observability（前端可观测性）
...
// Console errors — optional capture via override
...
| `error_displayed` | errorCode, errorMessage, componentName |

**Privacy rule:** Never log PII (field values, passwords, tokens). Log structural info only (field names, counts, error codes).

### 4.3 Performance Metrics (Web Vitals)

Collect Core Web Vitals for frontend performance monitoring:
...
Reported via the same `/api/logs` batch endpoint, tagged with `source: "client"` and `category: "web_vitals"`.

### 4.4 Frontend ↔ Backend Trace Correlation

The trace_id generated in the backend MUST be accessible to the frontend for end-to-end correlation:

```
Browser request
  → X-Trace-Id header (set by server or generated by client)
...
**Implementation:**
1. Server returns `X-Trace-Id` in response headers
2. Client stores current `traceId` per-request
3. All client logs and error reports include this `traceId`
4. Log search by `traceId` returns full client+server timeline

## 5. Log Lifecycle（日志生命周期）
...
To prevent runaway costs, enforce log volume budgets:

- Per-tenant daily log volume limit (configurable, default 100MB/day)
- Automatic sampling increase when approaching budget (reduce info → 50%, then 10%)
