...
| **结构** | KV vs 配置模型 | KV = 简单标量值；配置模型 = 结构化记录（`EntityDefinition` + `purpose: 'config'`） |
...
配置模型本质上就是一个带 `purpose: 'config'` 的 `EntityDefinition`：
...
- 作用域级联解析（全局 → 租户）
- 配置模型支持（`EntityDefinition` 上的 `purpose: 'config'`）
- 动态配置加密 + 脱敏集成（spec 41）
- cap-adapter-ui 中的管理员设置 UI
