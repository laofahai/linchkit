...
- 消除当前 `dev.ts` 中根据 DB 可用性重新装配 cap-auth 的 hack
...
**加密：** 标记 `encrypted: true` 的 KV 配置条目使用与业务数据字段相同的 `EncryptionProvider` 加密存储。`_linchkit_config.value` 列在标记时存储加密的 JSONB。
...
- `_linchkit_config` 系统表 + `DynamicConfigProvider`
