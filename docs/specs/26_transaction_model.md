...
>
> Related issues:
> - GitHub Issue `#123` — Transaction model
>
...
- 嵌套 Action 事务（§1.1 single-action transactions, nested via `ctx.execute`） — Implemented (issue #123). When an Action handler calls another Action through `ctx.execute()` while the parent is running inside an open transaction, the child shares the parent's transactional `DataProvider` (`_txDataProvider` seam) and merges its pending events into the parent's `_parentPendingEvents`. Either both commit, or both roll back — no SAVEPOINT, no per-child transaction. A `policy.transaction:false` parent does NOT have a transaction to share, so a child with `policy.transaction:true` opens its own. If the parent catches a failed child result and tries to keep going, the engine logs a `[nested-action]` warning via `Logger.warn` because the underlying SQL transaction is now on a rollback path and any subsequent write will fail. **Nested invocations are NOT separately tracked by Flow's compensation iterator** — only top-level steps participate in Saga rollback; intra-step nested writes share the parent step's tx and roll back atomically with it.
- 变更事务（蓝绿部署原子性）
- 幂等键管理
