...
| `schedule` | 按 cron 表达式定时评估 | 后台定时轮询（croner 计算下次到期） | ✅ 已实现（PR #439） |
...
`schedule` 按 cron 表达式定时评估条件（PR #439）。引擎用 `ScheduleTracker`（基于 `croner`，仅用于**计算**下次到期时间，不挂真实定时器）追踪每个 schedule watcher 的下次到期，由 `ScheduleEngine` 在内部 tick（默认 60s）上检查到期项并按其可选的 `count` 条件触发。tick 由可注入的时钟驱动，测试可推进时钟并直接调用 `runScheduleTick()` 来确定性地驱动调度。
...
- `'once_until_reset'` — 同一个 group key 内，条件再次满足时**不会**自动 re-fire；
  ...
  - 对 **`threshold` / `set_change`**：状态只会被 fire 时设为 `true`，不再回滚。后续若需 re-fire，必须显式调用 `WatcherEngine.resetState(name, groupKey)`。注意：当配置了持久化 `WatcherStateStore` 时，重启**不再**清空状态（这正是持久化的目的）；只有未配置 store 的纯内存模式才会因重启而重置。
...
- **轮询评估** —— `staleness` 触发器由 `setInterval`（默认 60s）周期性检查 `dataQuerier.queryRecords()` 的结果。`schedule` 触发器由 `ScheduleEngine` 在独立的 tick（默认 60s）上检查 croner 计算的到期时间并评估（见 §2.4）。
...
- **debounce 状态** —— 引擎内部维护一份内存 `stateMap`（key 为 `${watcherName}:${groupKey}`）作为**同步热缓存**。当装配时注入了 `WatcherStateStore` 时，每次状态变更都会**写穿（write-through）**到持久化后端，并在 `WatcherEngine.start()` / `hydrate()` 时从后端**回灌（hydrate）**缓存——因此进程重启后 debounce 状态可恢复，`once_until_reset` 已触发的 watcher 不会在重启后重复触发。
  - 默认实现 `InMemoryWatcherStateStore` 不跨进程持久化；**不配置 store 时引擎退化为纯内存 `stateMap`，行为与历史完全一致**（无任何 store 开销，重启仍会丢状态）。
  - 生产实现 `DrizzleWatcherStateStore` 落库到 `_linchkit.watcher_state` 系统表（主键 `(watcher_name, group_key)`，列含 `condition_met` / `last_fired_at` / `tenant_id` / `updated_at`）。表定义是 `addons/ai-provider/cap-ai-provider/src/watcher-state-table.ts` 里的 Drizzle `pgTable`（DDL 永远委托 drizzle-kit，绝不手写）；与其他 addon 拥有的 `_linchkit.*` 系统表（如 `mcp_clients`、`search_documents`）一致，通过 `db:push` 供给，**不进核心迁移链**。写穿为 fire-and-forget：store 写失败只记日志，不阻断（同步的）评估路径。

详见实现：`addons/ai-provider/cap-ai-provider/src/watcher-engine.ts`、`watcher-state-store.ts`、`watcher-state-store-drizzle.ts`、`watcher-state-table.ts`；测试：`addons/ai-provider/cap-ai-provider/__tests__/watcher-engine.test.ts`、`watcher-state-store.test.ts`、`watcher-state-store-drizzle.integration.test.ts`。
...
若改用 `'once_until_reset'`，则触发一次后状态会一直停留在「已触发」，直到调用 `WatcherEngine.resetState(...)` —— 适合「永久封存到首次告警结束」的场景，不适合自动恢复。（配置了持久化 store 时该「已触发」状态会跨重启保留；纯内存模式下重启会重置。）
...
实装依赖 `_linchkit.watcher_state` 表（§4 已落库，`DrizzleWatcherStateStore` + `watcher-state-table.ts` 表定义）和 cap-ai-provider 暴露的查询接口。**当前缺口**：尚未为 `watcher_state` 注册系统 `EntityDefinition`（adapter-server 的 `INTERNAL_SCHEMA_NAMES` 白名单 + SystemDataProvider 数据源），故详情页的「从 `watcher_state` 读取每个 group key 状态」仍待此 UI PR 实装。
...
- WatcherEngine 实装（`@linchkit/cap-ai-provider`）：threshold（单条 + 聚合）、set_change、staleness、schedule（cron 调度，croner 计算下次到期，PR #439）
- 三种 debounce 策略：`once_until_reset` / `once_per_record` / `cooldown`
- EventBus 订阅、CommandLayer 接入
- **debounce 状态持久化（重启安全）** —— `WatcherStateStore` 抽象 + `InMemoryWatcherStateStore`（默认）+ `DrizzleWatcherStateStore`（`_linchkit.watcher_state` 系统表，`watcher-state-table.ts` 定义，随 addon 经 `db:push` 供给）；引擎写穿 + 启动回灌，重启后 `once_until_reset` 不再重复触发。不配置 store 时保持纯内存历史行为。

### 待完成（M5+）

- `watcher_state` 的系统 `EntityDefinition` + SystemDataProvider 数据源（供 §7.1 详情页读取，本次仅落库未接 UI）
