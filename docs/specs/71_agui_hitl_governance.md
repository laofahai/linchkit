...
```
{
  ...
  toolCallId: <the proposeMutation tool-call id, reserved-prefixed: "lk:propose-mutation:<uuid>">,  // §4.5 — sentinel only used by the fallback synchronous drop; the primary suppression is server-side at the source so these frames are never emitted
...
- **Suppress the `proposeMutation` tool-call chunks at the SERVER source (REQUIRED).** Because `proposeMutation` is a *server-defined* tool (§4.3), its `TOOL_CALL_START` / `TOOL_CALL_ARGS` / `TOOL_CALL_END` events would otherwise stream to the client, and `createAgUiChunkTranslator` (`:197-262`) would translate them into `tool-input-start` / `tool-input-delta` / `tool-input-available` chunks — rendering a raw "Calling proposeMutation…" tool bubble in the chat stream *before* the `ActionProposalCard` even appears. `proposeMutation` is the **interrupt carrier, not a user-visible tool call.**
  - **Why client-side suppression is too late.** The naïve fix — having the translator key suppression on the interrupt's `toolCallId` — does **not** work: the `toolCallId` is only knowable from `RUN_FINISHED.outcome.interrupts[]`, which the client sees *after* every prior event. By the time the interrupt arrives, the `TOOL_CALL_START/ARGS/END` chunks have **already streamed and already rendered** the bubble. Suppression keyed on the interrupt (known only at run-end) cannot un-emit an already-shown bubble — it is retroactive, and the leak has already happened.
  - **Fix — server-side source suppression (preferred).** The **server** (the runner / `run-endpoint.ts`) MUST NOT emit `TOOL_CALL_START` / `TOOL_CALL_ARGS` / `TOOL_CALL_END` events for the `proposeMutation` tool-call at all. `proposeMutation` is an internal interrupt carrier; the runner **filters it out of the streamed tool-call events at the source** — the only place its arguments surface to the client is the interrupt outcome's `metadata` (§4.2). With no tool-call frames emitted, the translator never has a chunk to leak, so the bubble is impossible by construction (not suppressed-after-the-fact). The runner already owns which tool-call deltas it forwards to `emit`, so this is a source-level filter, not a post-hoc fix.
  - **Fallback — synchronous reserved-prefix match.** If a future constraint forces the `proposeMutation` tool-call frames to be emitted, its tool-call **id MUST carry a recognizable reserved prefix/sentinel** (e.g. `lk:propose-mutation:<uuid>`) that the translator matches **synchronously as each `TOOL_CALL_*` chunk arrives** — dropping the chunk on the spot, never via the after-the-fact interrupt id. This keeps suppression stream-time, not retroactive. Server-side source suppression is preferred; the sentinel is the contingency.
  - The proposal surfaces *only* as the `ActionProposalCard` rendered from the interrupt outcome; under either mechanism no raw tool bubble for `proposeMutation` may leak into the message stream.
...
4. **One-shot resume — synchronous claim before any `await`.** The store entry's `consumed` flag is claimed via an **atomic check-and-set** at the very top of the resume handler, **before any `await`** (see §6.7 for why a post-`await` CAS is not atomic). A replayed `resume` for an already-claimed interrupt loses the claim and is rejected (`RUN_ERROR`, no double-execute) — and the synchronous claim also serializes two concurrent resumes so only one proceeds to validation. If the subsequent async validation fails, the claim is **released/rolled back** so a legitimate retry is still possible (§6.7).
5. **Resume actor must match the PROPOSING actor — close cross-user resume.** Fail-closed-on-unauthenticated (§6.3) is necessary but **not sufficient**: it rejects anonymous/synthetic resumers, but it does NOT stop a *different authenticated* user (user B) from resuming user A's interrupt using A's `(threadId, interruptId)` — B passes the authenticated check yet never saw A's proposal. Defense: the interrupt store entry (§6.7) **binds the proposing actor and tenant captured on run A** (`proposerActor`, `tenant`), and the resume handler MUST **assert the re-resolved run-B actor identity equals the stored proposer** (same actor `id` *and* `tenant`) before executing. A mismatch is rejected — `RUN_ERROR`, **no `commandLayer.execute`** — *even though the resumer is a real authenticated actor*. Only the human who saw the proposal can approve it. (This is enforced against the store, server-authoritatively; the client-asserted actor is never trusted — §6.3.)
...
> **Fail-closed-on-unauthenticated is necessary but NOT sufficient — it does not close cross-user resume.** Rejecting anonymous/synthetic resumers still admits a *different authenticated* user (user B) resuming user A's `(threadId, interruptId)`: B is a real authenticated actor, so B clears this check, yet B never saw A's proposal. Closing that requires binding the interrupt to the **proposing** actor and asserting identity match on resume — specified as **§6.2 point 5** (resume actor must equal the stored `proposerActor` + `tenant`) and persisted in the store entry's `proposerActor`/`tenant` fields (§6.7). The two defenses compose: §6.3 rejects unauthenticated resumers; §6.2 point 5 rejects authenticated-but-wrong-user resumers.
...
Everything in §6.2 (synchronous one-shot claim, anti-TOCTOU action-set binding, digest anchoring, proposer-identity binding), §6.3 (the fail-closed write path's proposer/tenant capture), §6.4 (unknown/forgotten interrupt → `RUN_ERROR`), and the §5 expiry/abandon transitions assumes the server can, on run B, **look up the interrupt it emitted on run A** — yet run A's SSE stream and run B's resume request are **two separate, stateless HTTP connections** (§9.2 keeps the transport stateless; the run already *finished* per §3.1). The state that bridges them is never named elsewhere in this spec. It is the load-bearing piece, specified here.
...
InterruptStoreEntry {
  ...
  consumed:      boolean,                 // one-shot flag, claimed synchronously before any await (§6.2 point 4)
  proposerActor: ActorBinding,            // the run-A human actor identity ({ type, id }) — resume MUST match this (§6.2 point 5, §6.3)
  tenant:        string                   // the run-A tenant scope — resume MUST match this (§6.2 point 5, §6.3)
}
```

**Lifecycle.**
- **Written** when the interrupt outcome is emitted on run A (`RUN_FINISHED.outcome.type === "interrupt"`) — the entry is the server's record that this proposal is open. It captures `proposerActor` + `tenant` from run A's re-resolved human actor (§6.3) so run B can be checked against the human who actually saw the proposal (§6.2 point 5).
- **Claimed (synchronous) → validated (async) → executed-or-released** on run B's resume. The one-shot claim and the async validation must NOT be conflated into a single "CAS, then `await`" step, because in Bun/Node's single-threaded loop **only synchronous sections are atomic** — a CAS that reads `consumed`, then `await`s digest validation / actor resolution, then writes `consumed = true` is **not** atomic: two concurrent resumes can both pass the read before either writes, and both execute. So the claim must happen *before any `await`*:
  1. **Claim — synchronous, before any `await`.** Atomically read-and-set `consumed = true` (or remove the entry) for `(threadId, payload.interruptId)` in one synchronous critical section. If it was already `consumed`/absent → reject (`RUN_ERROR`, no execute). For the **in-process `Map`**: a plain synchronous `if (entry.consumed) reject; entry.consumed = true;` *with no `await` between the read and the write* is the atomic claim (the event loop cannot interleave a synchronous block). For the **durable Postgres** variant: a single conditional `UPDATE _linchkit.<table> SET consumed = true WHERE interrupt_id = $1 AND thread_id = $2 AND consumed = false RETURNING *` — a real atomic CAS at the DB — and **zero affected rows ⇒ already consumed/absent ⇒ reject**. The `RETURNING *` hands back the stored binding for the next step in the same round-trip.
  2. **Validate — async, after the claim is held.** Only now do the awaitable checks run: `payload.action ∈ actionSet`, `payload.baseDigest === inputDigest`, `now < expiresAt`, and the re-resolved run-B `proposerActor` + `tenant` equal the stored binding (§6.2 point 5, §6.3).
  3. **Execute or release.** On all checks passing → `commandLayer.execute` (§6.1). If **any** validation fails, the claim is **released/rolled back** — restore the entry (`consumed = false`, in-process) or mark it failed-but-not-consumed (durable: a compensating `UPDATE ... SET consumed = false WHERE interrupt_id = $1` once, scoped to this handler's own claim) — so a *legitimate* retry by the rightful proposer is still possible and a transient validation error doesn't permanently burn the one shot. A genuine reject (cancelled, or hard mismatch) evicts instead of releasing.
- **Evicted** on successful consume, on genuine rejection (cancelled / identity or digest mismatch), on expiry (`now ≥ expiresAt`), or on abandon (TTL sweep — the user disconnected without resuming, §5 *abandoned*). A *released* claim (transient validation failure) is NOT evicted — it returns to the open, unconsumed state for retry.

**A resume referencing an absent, expired, already-consumed, or wrong-proposer entry is rejected server-side** — `RUN_ERROR`, **no `commandLayer.execute`** — never executed on faith. This is the single enforcement point behind §6.2 points 1, 4, and 5, §6.4's "forgotten interrupt," and §5's `expired`/`abandoned` transitions.
...
Cross-referenced by **§5** (the `interrupted`/`expired`/`abandoned` transitions resolve against this store), **§6.2** (points 1–5 enforce against this store — including point 5's proposer-identity binding and point 4's synchronous one-shot claim), **§6.3** (the fail-closed write path + cross-user-resume defense persist `proposerActor`/`tenant` here), and **§6.4** (the unknown/forgotten-interrupt rejection reads this store).
...
| **P2 — server runner** | The runner exposes the execute-less `proposeMutation` server tool, captures a proposed mutation, and signals an interrupt outcome on run end; **filters the `proposeMutation` tool-call frames out of the streamed events at the source** (§4.5 — no `TOOL_CALL_START/ARGS/END` for it ever reaches `emit`, so no tool bubble can leak); the endpoint accepts `input.resume` and, on `status:"resolved"`, executes the approved Action through `commandLayer.execute` with the human actor + §6 bindings; on `cancelled`, finishes with no write. **Signature change (§4.3):** `AgUiAgentRunner`'s exported type changes from `=> Promise<void>` to `=> Promise<void \| AgUiInterruptDescriptor>` (return an optional interrupt descriptor); `run-endpoint.ts` consumes the returned descriptor to attach the `RUN_FINISHED.outcome`. Migrate all downstream runner implementations together. | `agui-runner.ts:283-369` (add propose tool + interrupt signal + resume-execute path; **filter `proposeMutation` tool-call deltas out of `emit` at the source**; **change the `AgUiAgentRunner` return type** to the optional interrupt descriptor; keep `allowActionExecution:false` for the existing read tools), `run-endpoint.ts:155-264` (consume the returned descriptor, resume branch, attach `outcome`), `tools.ts` (a dedicated `proposeMutation` builder, distinct from `executeAction`) | server test (`app.handle`): run A with a "create X" message yields `RUN_FINISHED` + interrupt outcome **and emits NO `TOOL_CALL_*` events for `proposeMutation`**; run B with `resume:resolved` executes via CommandLayer (assert the record exists, permission slot ran); `resume:cancelled` writes nothing; a swapped (outside the offered set)/expired interrupt is rejected |
| **P3 — UI transport + card** | Transport carries `resume[]` into `RunAgentInput`, adds an **interrupt branch** to `createAgUiChunkTranslator`; the `proposeMutation` tool bubble is already gone because P2 suppresses it at the server source (§4.5 — the translator never receives a `TOOL_CALL_*` for it), so P3 only adds the **fallback synchronous reserved-prefix drop** as defense-in-depth (match `lk:propose-mutation:*` ids as each chunk arrives, never via the after-the-fact `interrupt.toolCallId`); `ai-assistant.tsx` renders `ActionProposalCard` from the interrupt; Approve/Cancel emit `resume[]` (using `buildResumeArray`) instead of calling `executeAction`. | `agui-chat-transport.ts:368-383` (thread `resume`), `:197-262` (interrupt branch via `getRunOutcome` **+ fallback synchronous reserved-prefix drop** of any stray `proposeMutation` tool-call chunk — matched stream-time on the id sentinel, NOT retroactively on `interrupt.toolCallId`), `:386` (resume replaces the `reconnectToStream→null` stub or a parallel resume entry point), `ai-assistant.tsx` (render card from interrupt, wire Approve→resume), `action-proposal-card.tsx:288-308` (Execute raises a callback, no direct `executeAction`) | browser e2e (§8): real prompt → card renders from the interrupt → edit + Approve → record changes; Cancel → no change; **assert no raw `proposeMutation` tool bubble appears in the stream** |
