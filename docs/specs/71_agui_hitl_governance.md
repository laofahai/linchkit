...
sequenceDiagram
  ...
  UI->>EP: RunAgentInput { threadId, runId=B, resume:[{interruptId:I.id, status:"resolved", payload:{input:{...,price:8.9}}}] }
...
`payload.action` is echoed (defense-in-depth — the server re-derives it from the interrupt and asserts equality). `payload.input` is the **edited** input the human approved (identical to `proposedInput` when they didn't edit). `baseDigest` proves the client resumed *this* interrupt's proposal (§6.2).
...
- **interrupted → expired**: `expiresAt` (`InterruptSchema.expiresAt`) passed before a resume arrived. The client uses `isInterruptExpired` (`client .d.ts:620`) to disable Approve; a late resume for an expired interrupt is **rejected server-side** (the approval window is authoritative on the server — never trust the client clock).
- **interrupted → abandoned**: the user closes the panel / disconnects without resuming. No state leaks: because the run already finished, there is no open socket to leak; the proposed mutation was never executed. A later resume for an unknown/forgotten interrupt is rejected (§6.4).
...
The threat: between propose (run A) and execute (run B) the action name or inputs are swapped, so the human approves one thing and a different thing executes. Defenses, all **server-authoritative**:

1. **Resume must reference a live interrupt.** The server keys interrupts by `interrupt.id` (and `toolCallId`) for the `threadId`; a `ResumeEntry.interruptId` that doesn't match an open interrupt is rejected (`RUN_ERROR`, no execution).
2. **Action name is server-derived, not client-chosen.** The server re-reads `metadata.action` from the stored interrupt; `payload.action` is only asserted equal (defense-in-depth) — the client cannot substitute a different action by editing the payload.
...
4. **One-shot resume.** An interrupt is consumed on first resume; a replayed `resume` for an already-resolved interrupt is rejected (no double-execute).
