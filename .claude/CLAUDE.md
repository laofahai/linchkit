...
- **Agent Name:** builder-lint-fix
- **Task ID:** linchkit-9dbf
- **Spec:** No spec file provided
- **Branch:** overstory/builder-lint-fix/linchkit-9dbf
- **Worktree:** /Users/laofahai/Documents/workspace/linchkit/.overstory/worktrees/builder-lint-fix
- **Parent:** coordinator
- **Depth:** 1
...
Your worktree root is: `/Users/laofahai/Documents/workspace/linchkit/.overstory/worktrees/builder-lint-fix`

**CRITICAL**: All file operations MUST use paths within this directory.
- Use paths relative to your worktree root, or absolute paths starting with `/Users/laofahai/Documents/workspace/linchkit/.overstory/worktrees/builder-lint-fix`
...
These paths are relative to your worktree root: `/Users/laofahai/Documents/workspace/linchkit/.overstory/worktrees/builder-lint-fix`
...
Use `ov mail` for all communication. Your address is **builder-lint-fix**.

```bash
# Check your inbox (do this regularly)
ov mail check --agent builder-lint-fix

# Send a status update to your parent
ov mail send --to coordinator --subject "status" \
  --body "Progress update here" --type status --agent builder-lint-fix

# Ask a question
ov mail send --to coordinator --subject "question" \
  --body "Your question here" --type question --priority high --agent builder-lint-fix

# Report completion
ov mail send --to coordinator --subject "done" \
  --body "Summary of what was done" --type result --agent builder-lint-fix

# Reply to a message
ov mail reply <message-id> --body "Your reply" --agent builder-lint-fix
```
...
4. **Commit:** all changes committed to your branch (overstory/builder-lint-fix/linchkit-9dbf)
5. **Record mulch learnings:** `ml record <domain> --type <convention|pattern|failure|decision> --description "..." --outcome-status success --outcome-agent builder-lint-fix` — capture insights from your work
6. **Signal completion:** send `worker_done` mail to coordinator: `ov mail send --to coordinator --subject "Worker done: linchkit-9dbf" --body "Quality gates passed." --type worker_done --agent builder-lint-fix`
7. **Close issue:** `sd close linchkit-9dbf --reason "summary of changes"`
...
- **WORKTREE ISOLATION**: All writes MUST target files within your worktree at `/Users/laofahai/Documents/workspace/linchkit/.overstory/worktrees/builder-lint-fix`
- NEVER write to the canonical repo root — all writes go to your worktree copy
- Only modify files in your File Scope
- Commit only to your branch: overstory/builder-lint-fix/linchkit-9dbf
